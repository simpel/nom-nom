...
enum DS {
    enum Color {
        ...
        // MARK: - Pro (Hue ~267°) — reserved exclusively for Nom Nom Pro identity
        // (badges, gates, paywall). Never used for ordinary emphasis/interaction —
        // that stays `accent` — so violet reads as one unambiguous signal: "this is Pro."
        enum Pro {
            /// Pale wash for badge pills and gated-card backgrounds.
            static let proSoft = SwiftUI.Color("ds/pro/proSoft")
            /// Hairline border on `proSoft` surfaces (badge pill, gated-card frame).
            static let proBorder = SwiftUI.Color("ds/pro/proBorder")
            /// Icon tint and text color on `proSoft` surfaces.
            static let proAccent = SwiftUI.Color("ds/pro/proAccent")
            /// Deep solid fill for high-emphasis Pro CTAs (e.g. "Unlock with Pro").
            static let proDeep = SwiftUI.Color("ds/pro/proDeep")
        }

        // MARK: - Chart Categorical Palette
        // Fixed-order, CVD-validated hues for identity encoding (e.g. one line per
        // party member on a trend chart). Assign by stable index, never cycled or
        // reassigned when the series count changes. Violet is deliberately excluded —
        ...
        enum Chart {
            static let series: [SwiftUI.Color] = [
                ...
                SwiftUI.Color("ds/chart/series3"),
                SwiftUI.Color("ds/chart/series4"),
                SwiftUI.Color("ds/chart/series5"),
                SwiftUI.Color("ds/chart/series6"),
                SwiftUI.Color("ds/chart/series7"),
            ]
        }
