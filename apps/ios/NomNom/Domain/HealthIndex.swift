...
struct HealthIndex: Codable, Hashable, Equatable {
    ...
    /// LLM-normalized core ingredient categories (e.g. "chicken breast" and "chicken
    /// thighs" both become "chicken") — lets rater-affinity analysis group ingredient
    /// variants without a hardcoded keyword list. Empty for recipes analyzed before this
    /// field existed, backfilled the same way `score`/`verdict`/etc. are.
