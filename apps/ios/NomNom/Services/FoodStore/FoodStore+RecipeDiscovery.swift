...
extension FoodStore {
    ...
    /// Top recipes from `SuggestionEngine`'s multi-factor ranker (likes, overdue-ness,
    /// exploration, weekday affinity) — a Pro "Recommended for You" surface.
    var recommendedRecipes: [Recipe] {
        SuggestionEngine().rank(suggestionInputs, filters: SuggestionFilters())
            .prefix(10)
            .map(\.dish)
    }
