...
def generate_category_image(resolved_cat, resolved_dishes, vessels_narrative=None):
    ...
    image_prompt = (
        f"A professional editorial food photograph showcasing the essence of {resolved_cat}, "
        ...
        f"light-to-medium honey-beige wood surface with absolutely no plank boards, no joints, no seams, no grooves, "
        f"and no gaps anywhere; the grain of the wood runs strictly horizontally from left to right across the frame, "
        f"with fine, flat, straight-to-gently-wavy grain lines flowing horizontally from left to right, with no change "
        ...
        f"visible anywhere — strictly wood surface and plates only. Shot from an elevated 3/4 angle (~45 degrees top-down angle) "
        f"looking down, cohesive multi-plate spread composition occupying 75% of the frame with clean wood negative space "
        f"around it. A single large, heavily diffused natural daylight source enters from the upper-left at roughly a 45-degree "
